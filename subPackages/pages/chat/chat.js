...
Page({
  ...
  toUserSendMsg(e) {
    ...
    // 判断对方是否离线
    let isOffline = this.data.onlineUserList.find(
      (item) => item.id == this.data.chatObj.id
    );
  ...
  onUnload() {
    ...
    // 将未读数据移除
    let newUnreadList = this.data.unreadList.filter(
      (item) => item != this.data.chatObj.id
    );
